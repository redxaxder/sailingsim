...
impl Heading {
    pub fn interpolate(self, rhs: Heading) -> Option<Vec<Heading>> {
        let r: Vec<Heading> = self.0.interpolate(rhs.0)?.drain(..).map(Heading).collect();
        Some(r)
    }
}
...
impl Maneuver {
    ...
    pub fn cost(w: Wind, h1: Heading, h2: Heading) -> u8 {
        ...
        match intermediate_headings {
            ...
            Some(mut hs) => {
                let mut cost = 0;
                let poss: Vec<PointOfSail> = hs.drain(..).map(|h| PointOfSail::pos(h, w)).collect();
...
fn move_ship(
    ...
    if let (Ok((mut p, mut h, mut m)), Ok(&wind)) = (query.single_mut(), wind_q.single()) {
        ...
        for &action in e.iter() {
            ...
            if cost <= m.0 {
                ...
                if cost == 0 {
                    // if no maneuver cost, recover a point
                    m.0 = (m.0 + 1).min(10);
                }
...
fn update_display_position(mut query: Query<(&mut Transform, &Position), Changed<Position>>) {
...
fn setup(
    ...
    // Wind
    commands.spawn().insert(Wind(Dir::RIGHT));
...
fn get_movement_input(mut key_evr: EventReader<KeyboardInput>, mut w: EventWriter<Action>) {
    ...
    for ev in key_evr.iter() {
        if let (ElementState::Pressed, Some(code)) = (ev.state, ev.key_code) {
            match code {
                KeyCode::Right | KeyCode::D | KeyCode::L => w.send(Action::Move(Dir::RIGHT)),
                KeyCode::Left | KeyCode::A | KeyCode::H => w.send(Action::Move(Dir::LEFT)),
                KeyCode::Up | KeyCode::W | KeyCode::K => w.send(Action::Move(Dir::UP)),
                KeyCode::Down | KeyCode::S | KeyCode::J => w.send(Action::Move(Dir::DOWN)),
                KeyCode::Q | KeyCode::Y => w.send(Action::Move(Dir::UPLEFT)),
                KeyCode::E | KeyCode::U => w.send(Action::Move(Dir::UPRIGHT)),
                KeyCode::Z | KeyCode::B => w.send(Action::Move(Dir::DOWNLEFT)),
                KeyCode::C | KeyCode::N => w.send(Action::Move(Dir::DOWNRIGHT)),
                KeyCode::Space => w.send(Action::Wait),
                _ => {}
            }
        }
    }
